      ...
      <footer className={`${styles.paddingX} flex flex-col  bg-black`}>
         <div className='flex flex-col sm:flex-row justify-evenly sm:justify-between items-start sm:items-center h-[120px] my-20 xs:my-0'>
            ...
            <ul className='flex flex-wrap flex-col xs:flex-row'>
               {navLinks.map((link) => (
                  <li className={`${styles.body1} mr-[24px] text-white mt-2 xs:mt-0`} key={link.id}><Link to={link.path}>{link.title}</Link>
                  </li>
               ))}
            </ul>
         </div>
         ...
         <div className={`${styles.paddingX} py-[64px] flex flex-col md:flex-row wrap items-baseline justife-between bg-[#2f303f]`}>
            <h2 className={`${styles.heading2} mr-5 md:mb-0 mb-10`}>Subscribe to our news letter to get <br /> latest updates and news</h2>
            <div className={`flex flex-col xs:flex-row items-start`}>
               <input type="text" placeholder='Enter Your Email' className={`${styles.body1} max-w-[500px] sm:max-w-[320px] xs:max-w-[200px] flex-shrink-2 py-[14px] px-[25px] bg-transparent border-[1px] border-[#6D6E76] mb-5 xs:mb-0 mr-5 text-white`} />
         ...
         <div className='flex flex-row items-center h-[140px]'>
            <div className='flex flex-col md:flex-row w-full justify-between'>
               <div className='flex flex-col'>
                  <p className={`font-thin font-inter text-[16px] text-white leading-[28px]`}>Finstreet 118 2561 Fintown</p>
                  <p className={`font-thin font-inter text-[16px] text-white leading-[28px]`}>Hello@finsweet.com  020 7993 2905</p>
               </div>
