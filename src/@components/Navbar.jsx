...
const Navbar = () => {
   ...
   return (
      <nav className={`${styles.paddingX} ${isScrollingDown ? 'opacity-90' : 'opacity-100'} delay-200 flex flex-row justify-between items-center navbar h-[80px] bg-black`}>
         ...
         <div className='sm:hidden'><img src={toggle ? close : menu} alt="menu" className='w-[28px] h-[28px] object-contain' onClick={() => setToggle((prev) => !prev)} /></div>

         <div className={`${toggle ? 'flex' : 'hidden'} absolute items-center top-[70px] right-0 sm:hidden px-4 sidebar rounded-xl bg-black`}>
